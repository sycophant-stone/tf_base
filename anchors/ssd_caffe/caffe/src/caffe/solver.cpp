...
void Solver<Dtype>::TestDetection(const int test_net_id) {
  ...
  for (int i = 0; i < param_.test_iter(test_net_id); ++i) {
    ...
    if (param_.test_compute_loss()) {
      loss += iter_loss;
	  LOG(INFO)<<"test_compute_loss, loss:"<<loss<<", iter_loss:"<<iter_loss;
    }
    for (int j = 0; j < result.size(); ++j) {
	  //LOG(INFO)<<"result, j:"<<j<<"/result.size():"<<result.size();
      CHECK_EQ(result[j]->width(), 5);
      const Dtype* result_vec = result[j]->cpu_data();
      int num_det = result[j]->height();
	  //LOG(INFO)<<"result["<<j<<"]"<<"width:"<<result[j]->width()<<", height:"<<result[j]->height();
      for (int k = 0; k < num_det; ++k) {
		//LOG(INFO)<<"result["<<j<<"].cpudata(): "<<"[0]:"<<result_vec[k*5]<<"[1]"<<result_vec[k*5+1]<<"[2]"<<result_vec[k*5+2]<<"[3]"<<result_vec[k*5+3]<<"[4]"<<result_vec[k*5+4];
        int item_id = static_cast<int>(result_vec[k * 5]);
        int label = static_cast<int>(result_vec[k * 5 + 1]);
        if (item_id == -1) {
          // Special row of storing number of positives for a label.
          if (all_num_pos[j].find(label) == all_num_pos[j].end()) {//map<int, map<int, int> >   map<int, int>  int <== score [2]
            all_num_pos[j][label] = static_cast<int>(result_vec[k * 5 + 2]);
          } else {
            all_num_pos[j][label] += static_cast<int>(result_vec[k * 5 + 2]);//								+=  score [2]
          }
        } else {
  ...
  for (int i = 0; i < all_true_pos.size(); ++i) {
    ...
    for (map<int, int>::const_iterator it = num_pos.begin();
      ...
      vector<float> prec, rec, max_precision;
      ComputeAP(label_true_pos, label_num_pos, label_false_pos,
                param_.ap_version(), &prec, &rec, &(APs[label]), &max_precision);
      mAP += APs[label];
      if (param_.show_per_class_result()) {
        LOG(INFO) << "class" << label << ": " << APs[label];
		if (param_.show_precisons_value()){
			for (int i=0;i<max_precision.size();i++){
				LOG(INFO)<<"max_precision["<<i<<"]="<<max_precision[i];
			}
		}
      }
    }
