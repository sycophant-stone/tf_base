...
void ComputeAP(const vector<pair<float, int> >& tp, const int num_pos,
               ...
               vector<float>* prec, vector<float>* rec, float* ap, vector<float>* temp_max_precision) {
  ...
  *ap = 0;
  ...
  if (ap_version == "11point") {
    ...
    int start_idx = num - 1;
	LOG(INFO) << "num:" << num<<", start_idx:"<<start_idx;
    for (int j = 10; j >= 0; --j) {
      for (int i = start_idx; i >= 0 ; --i) {
		//LOG(INFO) << "i:" << i<<" ,j:"<<j<<" ,rec[i]:"<<(*rec)[i]<<" ,prec[i]: "<<(*prec)[i];
        if ((*rec)[i] < j / 10.) {
          start_idx = i;
          if (j > 0) {
    ...
    for (int j = 10; j >= 0; --j) {
      *ap += max_precs[j] / 11;
	  temp_max_precision->push_back(max_precs[j]);
    }
