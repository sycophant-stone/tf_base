...
lr = 0.0001
batch_size = 1#32  # test 
buffer_size = 1024
epochs = 300
reduce_lr_epoch = []
ckpt_path = os.path.join('.', './pretrained/vgg_16.ckpt')
config = {
    'mode': 'test',                            # 'train' ,'test' , 'test'
...
refinedet = net.RefineDet320(config, testset_provider)
refinedet.load_weight('refinedet320/test-26052')

img = io.imread('000026.jpg')
img = transform.resize(img, [320,320])
img = np.expand_dims(img, 0)
result = refinedet.test_one_image(img)
